...
if exist "%ROOT%server\lib" (
    call :copy_dir "%ROOT%server\lib" "%OUTPUT_SERVER_DIR%\lib"
    if errorlevel 1 goto :fail
    REM 删除打包源码归档文(wawoff2.tgz)，节省空间
    if exist "%OUTPUT_SERVER_DIR%\lib\wawoff2.tgz" del /Q "%OUTPUT_SERVER_DIR%\lib\wawoff2.tgz" >nul 2>nul
)
if exist "%ROOT%server\scripts" (
    call :copy_dir "%ROOT%server\scripts" "%OUTPUT_SERVER_DIR%\scripts"
    if errorlevel 1 goto :fail
)
...
if not exist "%OUTPUT_SERVER_DIR%\lib\aBogus.js" (
    echo [ERROR] Missing packaged server lib: server\lib\aBogus.js
    goto :fail
)
if not exist "%OUTPUT_SERVER_DIR%\lib\fanqieFontDecoder.js" (
    echo [ERROR] Missing packaged server lib: server\lib\fanqieFontDecoder.js
    goto :fail
)
if not exist "%OUTPUT_SERVER_DIR%\lib\macGlyphNames.js" (
    echo [ERROR] Missing packaged server lib: server\lib\macGlyphNames.js
    goto :fail
)
if not exist "%OUTPUT_SERVER_DIR%\lib\puaMap.js" (
    echo [ERROR] Missing packaged server lib: server\lib\puaMap.js
    goto :fail
)
if not exist "%OUTPUT_SERVER_DIR%\lib\ttfParser.js" (
    echo [ERROR] Missing packaged server lib: server\lib\ttfParser.js
    goto :fail
)
if not exist "%OUTPUT_SERVER_DIR%\lib\wawoff2\index.js" (
    echo [ERROR] Missing packaged server lib: server\lib\wawoff2\index.js
    goto :fail
)
if not exist "%OUTPUT_SERVER_DIR%\routes\ai.js" (
    echo [ERROR] Missing packaged route: server\routes\ai.js
    goto :fail
)
if not exist "%OUTPUT_SERVER_DIR%\routes\apiConfig.js" (
    echo [ERROR] Missing packaged route: server\routes\apiConfig.js
    goto :fail
)
if not exist "%OUTPUT_SERVER_DIR%\routes\backup.js" (
    echo [ERROR] Missing packaged route: server\routes\backup.js
    goto :fail
)
if not exist "%OUTPUT_SERVER_DIR%\routes\books.js" (
    echo [ERROR] Missing packaged route: server\routes\books.js
    goto :fail
)
if not exist "%OUTPUT_SERVER_DIR%\routes\chapters.js" (
    echo [ERROR] Missing packaged route: server\routes\chapters.js
    goto :fail
)
if not exist "%OUTPUT_SERVER_DIR%\routes\characters.js" (
    echo [ERROR] Missing packaged route: server\routes\characters.js
    goto :fail
)
if not exist "%OUTPUT_SERVER_DIR%\routes\conversations.js" (
    echo [ERROR] Missing packaged route: server\routes\conversations.js
    goto :fail
)
if not exist "%OUTPUT_SERVER_DIR%\routes\entries.js" (
    echo [ERROR] Missing packaged route: server\routes\entries.js
    goto :fail
)
if not exist "%OUTPUT_SERVER_DIR%\routes\experienceShares.js" (
    echo [ERROR] Missing packaged route: server\routes\experienceShares.js
    goto :fail
)
if not exist "%OUTPUT_SERVER_DIR%\routes\fanqie.js" (
    echo [ERROR] Missing packaged route: server\routes\fanqie.js
    goto :fail
)
if not exist "%OUTPUT_SERVER_DIR%\routes\generators.js" (
    echo [ERROR] Missing packaged route: server\routes\generators.js
    goto :fail
)
if not exist "%OUTPUT_SERVER_DIR%\routes\history.js" (
    echo [ERROR] Missing packaged route: server\routes\history.js
    goto :fail
)
if not exist "%OUTPUT_SERVER_DIR%\routes\knowledgeGraph.js" (
    echo [ERROR] Missing packaged route: server\routes\knowledgeGraph.js
    goto :fail
)
if not exist "%OUTPUT_SERVER_DIR%\routes\memos.js" (
    echo [ERROR] Missing packaged route: server\routes\memos.js
    goto :fail
)
if not exist "%OUTPUT_SERVER_DIR%\routes\prompts.js" (
    echo [ERROR] Missing packaged route: server\routes\prompts.js
    goto :fail
)
if not exist "%OUTPUT_SERVER_DIR%\routes\providers.js" (
    echo [ERROR] Missing packaged route: server\routes\providers.js
    goto :fail
)
if not exist "%OUTPUT_SERVER_DIR%\routes\stats.js" (
    echo [ERROR] Missing packaged route: server\routes\stats.js
    goto :fail
)
if not exist "%OUTPUT_SERVER_DIR%\routes\volumes.js" (
    echo [ERROR] Missing packaged route: server\routes\volumes.js
    goto :fail
)
if not exist "%OUTPUT_SERVER_DIR%\routes\workflows.js" (
    echo [ERROR] Missing packaged route: server\routes\workflows.js
    goto :fail
)
if not exist "%OUTPUT_SERVER_DIR%\utils\progressBar.js" (
    echo [ERROR] Missing packaged util: server\utils\progressBar.js
    goto :fail
)
if not exist "%OUTPUT_SERVER_DIR%\utils\backupFormat.js" (
    echo [ERROR] Missing packaged util: server\utils\backupFormat.js
    goto :fail
)
if not exist "%OUTPUT_SERVER_DIR%\services\bookStorage.js" (
    echo [ERROR] Missing packaged service: server\services\bookStorage.js
    goto :fail
)
